...
def merkle_assignment():
    ...
    random_leaf_index = random.randint(0,len(primes)-1)  #TODO generate a random index from primes to claim (0 is already claimed)
...
def generate_primes(num_primes):
    ...
    while len(primes_list) < num_primes:
        is_prime = all(num % prime != 0 for prime in primes_list)
        if is_prime:
